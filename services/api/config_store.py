...
LOGGER = logging.getLogger("cachefront.api.config")
...
def save_runtime_config(payload: dict[str, Any]) -> dict[str, str]:
    normalized = {"mode": normalize_cache_mode(payload.get("mode"))}
    path = default_config_path()
    with _CONFIG_LOCK:
        _write_runtime_config(path, normalized)
    return normalized
...
def set_cache_mode(mode: str) -> dict[str, str]:
    return save_runtime_config({"mode": mode})
